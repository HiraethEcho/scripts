...
dwm_resources () {
  ...
  TOTAL_RAM=$(free -mh --si | awk  {'print $2'} | head -n 2 | tail -1)
	...
	if [ "$CPU_TEMP" -ge $WARNING_LEVEL ]; then
		printf "󰍛%s/%s󰘚%s󰈸%s°C󰋊%s/%s%s" "$USED_RAM" "$TOTAL_RAM" "$CPU" "$CPU_TEMP" "$STOUSED" "$STOTOT" "$STOPER"
  else
		printf "󰍛%s/%s󰘚%s%s°C󰋊%s/%s%s" "$USED_RAM" "$TOTAL_RAM" "$CPU" "$CPU_TEMP" "$STOUSED" "$STOTOT" "$STOPER"
	fi
}
